...
1. 在自由模式裡照平常方式新增一個頻道視窗,或加入語音室小工具。
2. 點該視窗標題列上的 📌 圖示(在最小化、關閉按鈕旁邊)。按下去之後圖示會亮紫色,代表已加入 overlay。
3. 在任何地方按 **`Ctrl+Shift+\``**(不需要 focus 在 Discord 上)就能整組顯示/隱藏所有已疊加的 overlay。

每個疊加的視窗都會有自己獨立的 overlay,位置跟大小會即時對齊你在自由模式裡那個視窗的實際狀態
(拖曳或縮放自由模式的視窗,overlay 也會跟著動),而且會自動夾在螢幕可視範圍內,不會跑到看不到
的地方去。頻道 overlay 會顯示最後 8 則訊息(唯讀),下面還有一個輸入框——直接打字按 Enter 就能
送出訊息,不用切回 Discord。語音室 overlay 會顯示目前通話的成員、說話中的綠框提示,以及麥克風/
耳機的靜音狀態。

這是真正的作業系統層級視窗(不是掛在 DirectX 或遊戲引擎上的 hook),所以只能疊在無邊框全螢幕或
視窗化模式的遊戲上,對獨佔全螢幕模式沒有作用——跟 Discord 自己的官方 overlay 有一樣的限制,無法
畫在第三方全螢幕擷取畫面裡面。
